Feature: cast an ability
  ...
  Scenario: Cast an ability
    ...
    And the battle unit has enough mana
    ...
    And the battle unit mana points are reduced based on the ability cost
    And the battle unit ability cooldown turns left is incremented based on the ability cooldown

Scenario: Battle unit does not exist
  Given a battle unit id that is not deployed
  And an ability id
  And a list of targeted battle unit ids
  When casting an ability
...
Scenario: Battle unit does not have enough mana
  Given a battle unit id
  And an ability id
  And the battle unit has less mana than the ability cost
